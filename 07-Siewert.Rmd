...
3) [Direct Development](#siewert-direct-dev)
    ...
    * See the 2 methods for calculating the $^{XS}LDF^L_t$
...
5) [Development Method](#siewert-dev-method)
    ...
    iii. Severity LDF formulas, know them well to manipulate and know what formula requires what
    
        Proposition \@ref(prp:siewert-ldf-rel-1), \@ref(prp:siewert-ldf-rel-2), \@ref(prp:siewert-ldf-rel-3), \@ref(prp:inc-ldf-rel-1), and \@ref(prp:inc-ldf-rel-2)

6) [Distribution Method](#siewert-dist-model)

Finally there's the method for accounting for aggregate charge
...
3. Calculate $^{XS}LDF^L_t$ based on $R^L_t$
...
To be included later...(Not crucial)
```
