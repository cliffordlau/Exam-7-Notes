...
| $c(w,d)$  | Cum losses for AY $w$ age $d$ |
...
## 6 Testable Implications of Assumptions
...
Check if the coefficient is $> 2 \times \sigma$ for 95% sure they are $\neq 0$
...
Error term: $\varepsilon(w,d) = q(w,d) - f(d)h(w)$
...
* Weights will be $\dfrac{1}{f^p h^q}$
...
| Method | $\mathbf{\operatorname{Var}(q)}$ | $\mathbf{f(d)}$ | $\mathbf{h(w)}$ |
| --- | --- | --- | --- |
| BF w/ Constant Var | $a(d); p=q=0$ | $f(d) = \dfrac{\sum_w h^2 \frac{q}{h}}{\sum_w h^2}$ | $h(w) = \dfrac{\sum_d f^2 \frac{q}{f}}{\sum_d f^2}$ |
| Cape Cod | $a(d); p=q=0$ | $f(d) = \dfrac{\sum_w h^2 \frac{q}{h}}{\sum_w h^2}$ | $h = \dfrac{\sum_\Delta f^2 \frac{q}{f}}{\sum_\Delta f^2}$ |
| BF w/ Var $\propto$ $fh$| $a(d)\:fh; p=q=1$ | $f^2(d) = \dfrac{\sum_w h (\frac{q}{h})^2}{\sum_w h}$ | $h^2(w) = \dfrac{\sum_d f (\frac{q}{f})^2}{\sum_d f}$ |
...
* Need to seed on of them and iterate until convergence
...
* $n = \sum_i+1^{m-1} i = \dfrac{m(m-1)}{2}$ = predicted data point?
...
Calculate Pearso correlation for every pair of columns with at least 3 LDFs
...
Test statistics: $T = r \left[ \dfrac{n-2}{1-r^2} \right]^{1/2}$
