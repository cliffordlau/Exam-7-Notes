...
$\begin{align}
  \hat{f_k} = \frac{\sum_j \frac{c_{j,k+1}}{c_{j,k}} \: w_{j,k}}{\sum_j w_{j,k}}
  \end{align}$
...
$\begin{align}
  MSE(\hat{c}_{i,I}) = \hat{c}_{i,I}^2 \left \{ \sum_{k = I + 1 - i}^{I-1} \frac{\hat{\alpha}_k^2}{\hat{f_k}^2} \left ( \frac{1}{\hat{c}_{i,k}} + \underbrace{\frac{1}{\sum_{j=1}^{I-k}c_{j,k}}}_{\text{Column x latest}}\right ) \right \}
\end{align}$
...
* $\operatorname{E}[z_n] = \dfrac{n}{2} - c_n$ <span style="color:red;background-color:yellow">Memorize Formulas</span>
    ...
    * $n =$ # of elements in each diagonal
...
* Rank the LDFs for each column then take the difference^2^
...
* $T_k = 1 - \dfrac{S}{n(n^2-1)/6}$
    ...
    * For a 10 x 10 triangle, $k=7$ because there's only 9 LDFs so 8 pairs. And down to 7 because we don't use the pair with only 1 value
