...
| $\mathrm{Cov(X,Y)} \ll \mathrm{Var(X)}$ | $L(X) \approx \mathrm{E[Y]}$; X, Y dont' vary tightly together|
...
* Resutls sensitive to $\mathrm{E[Y]}$ and $\mathrm{E[D]}$ but not the $\sigma$
...
* Or the opposite whne large $y$ has as larger $d$: For a property book when large weather event happens, report quicker
...
Normalize the losses by dividing with premium since LS assume constant distribution

Calculates ATU, so needs the tail factor first and also start from the oldest period

Recusively go backwards using the previous estimates
