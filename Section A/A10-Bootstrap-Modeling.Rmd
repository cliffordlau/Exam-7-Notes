...
![alt text](figures/Exam 7 Notes - 1.png)
...
* Replace GLM fitting with much simplier calculation
...
Pinheiro proposed to standarized residuals so they all have same variance
...
* Convert utlimate from incurred to a payment stream based on paid analysis
* Correlate the simluations from the 2 method so that if we have large payment @ an older age, the incurred should be large as well
...
* Need to make use the absoluate value for the residual and resampling formula:
...
* But beware of understaing the the likelihood of extreme outcomes
...
* Exclude in LDFs but continue to use them when calculating residuals and use them for the resampling
...
Split the triangle into groups with similar variance and only sample residuals that are in ther group
...
Hetero-adjustment factor = $h^t$ = the largest $\sigma$ $\div$ each group's $\sigma$
...
Only want partial accient year

No impact to residuals for bootstrap

2 options:

* Reduce the mean of the incremental cells by proration in the process variance step
...
* GLM
    ...
    * Adjust the exposure in the last diagonal to make them consistent with the rest of the triangle (probably means adjusting annualing the loss)

Then prorate the losses simiar to the first scenario
...
Remove true outliers but do not want to remove points that are realistics extreme scenarios
