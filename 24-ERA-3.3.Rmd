...
[Pearson's](#era-3-3-pearson) correlation: Formula and its properties

* Outliers will have disproportionate weight

[Kendall's](#era-3-3-kendall) $\tau$: depends on the rank

* Know the formula \@ref(eq:era-3-3-ken) for discrete and continuous

**Copulas**

Know the limitation of joint distribution and [advantages](#era-3-3-copula-advantages) for using copulas
...
How to use a copula and Sklar's Theorem \@ref(thm:sklar)
...
Know the properties of the main copulas \@ref(tab:era-3-3-copulas-summary)

* There's also the partial perfect correlation copula

Know how the **simulation** is done for each of the copula

* [Frank](#era-3-3-sim-1), [Gumble](#era-3-3-sim-2), HRT is same as Frank, and [Normal](#era-3-3-sim-3)

**Tail Concentration Functions**

[Left tail](#left-tail-con-fun) and [right tail](#right-tail-con-fun) concentration function

* Know what each copula looks like
...
\begin{equation}
  (u,v) = \left( \\exp\left[ \\ln(s)r^{\frac{1}{a}} \right], \\exp\left[ \\ln(1-r)r^{\frac{1}{a}} \right] \right)
  (\#eq:era-gumbel-s-u-v)
\end{equation}
...
Haven't done past exam questions for ERA 3
```
