...
* Net cost of reinsurance should still be $\text{Reinsurance Premium} - \operatorname{E}[Recovery]$ on a PV basis
...
Earnings = Net Premium - Expenses - risk metric

We can then select the economic capital = earnings @ a certain risk metric (like $TVaR$)
...
Easier to implement (no need to model loss dist^n^ and correlation) but is not as accurate because the measurement of risk is based on proxies and not the actual risk itself
...
### Reinsurance, Capital and Accumulation Risk

So far we focused on a single AY or u/w year and did not account for the fact that losses maybe paid more than a year later or longer
...
* Calculate the reserves as if the company had been writing the same book the last several years and adjusted for inflation
...
2) Reinsurance can be applied to the current AY and as-if reserves providing more valid measure of the accumulated risk and capital used over the full life of the AY
...
Goal of a firm is to increase market value of the firm

* Given different options, will choose the one that increase the value the most on risk-adjusted basis

* Don't know how this can be done

Current step: measure value of the stability that reinsurance provides by measuring the reduction in capital  
$\Downarrow$  
Next step: Measure the value of reinsurance by measuring the $\Delta$ in the value of the company

* Holy grail of cost benefit analysis

Some conclusion from recent studies
...
* Value of reinsurance can be measures vs a cost of capital or ROE
