...
$\hat{\alpha}_{I-1}^2 = \mathrm{min} \left( \dfrac{(\hat{\alpha}_{I-2}^2)^2}{\hat{\alpha}_{I-3}^2},\hat{\alpha}_{I-3}^2 \right) = \begin{cases}
...
* Test 95% CI: $\mathrm{E}[Z] \pm 2 \times \sigma$
...
* $\mathrm{E}[T] \pm Z \sqrt{\mathrm{Var}(T)}$
...
\begin{equation}
  \varepsilon_{i,k} = \dfrac{c_{j,k+1} - c_{j,k} \: \hat{f_k}}{\sqrt{\mathrm{Var}(c_{j,k})}}
  (\#eq:residual-eq)
\end{equation}
