...
* Log-normal: equation \@ref(eq:lognormal-ci)
...
$$\operatorname{E}\left [c_{i,k+1} \mid c_{i,1} \cdots c_{i,k}\right ] = c_{i,k} \times f_k$$
```
...
Table: (\#tab:CL-var-weight) Relationships between weight, variance and residual
...
* Not necessarily assumption 2 (Prop. \@ref(prp:cl-ass-1))
...
Appling the properties of $\mathrm{Var}(X) = \mathrm{E}[X^2] - \mathrm{E}[X]^2$ we get
...
$R_i$ is the unpaid for year $i$: $R_i = c_{i,I} - c_{i,k}$

So the estimate of the unpaid claim is $\hat{R}_i = \hat{c}_{i,I} - c_{i,k}$
...
\begin{equation}
\hat{\alpha}_{I-1}^2 = \operatorname{min} \left( \hat{\alpha}_{I-2}^2 \times \dfrac{\hat{\alpha}_{I-2}^2}{\hat{\alpha}_{I-3}^2},\hat{\alpha}_{I-3}^2 \right) = \begin{cases}
  \hat{\alpha}_{I-3}^2 & \text{if } \hat{\alpha}_{I-3}^2 < \hat{\alpha}_{I-2}^2 \\
  \hat{\alpha}_{I-2}^2 \times \dfrac{\hat{\alpha}_{I-2}^2}{\hat{\alpha}_{I-3}^2} & \text{else}\\
\end{cases}
(\#eq:alpha-2)
...
\begin{equation}
  [s.e.(\hat{R})]^2 = \sum \limits_{i=2}^I \left\{ [s.e.(\hat{R}_i)]^2 + \hat{c}_{i,I} \left( \sum \limits_{j = i+1}^{I} \hat{c}_{j,I} \right) \left( \sum \limits_{k = I + 1 - i}^{I - 1} \dfrac{2 \hat{\alpha}^2_k \big/ \hat{f}^2_k}{\sum_{n=1}^{I-k}c_{n,k}}\right) \right\}
  (\#eq:mse-total)
\end{equation}
...
* Can be high for older years since reserve is small but absoluate s.e. is also small $\therefore$ not important
...
* Recall from previous section we mentioned that we should use log-normal distributio for the CI if $s.e.(\hat{R}_i) > \dfrac{R_i}{2}$, this is equivalent to $\dfrac{s.e.(\hat{R}_i)}{\hat{R}_i} > 50%$
...
4. Correlation of adjacent LDFs
...
For each age $k$, plot the $c_{i,k}$ with the residuals $\varepsilon_{i,k}$

\begin{equation}
  \varepsilon_{i,k} = \dfrac{c_{j,k+1} - c_{j,k} \: \hat{f_k}}{\sqrt{\operatorname{Var}(c_{j,k})}}
  (\#eq:residual-eq)
\end{equation}
...
* We can take out the $\alpha^2_k$ term since it's constant for the same $k$
    ...
    e.g. $\varepsilon = \dfrac{c_{j,k+1} - c_{j,k} \: \hat{f_k}}{\sqrt{c_{j,k}}}$ for weighted average assumption
...
### Calendar Year Test{#cy-test}

Test for assumption 1 (prop. \@ref(prp:cl-ass-2))
...
### Correlation of Adjacent LDFs{#adjacent-ldf}

Test assumption (1) (prop. \@ref(prp:cl-ass-1)) where expected only depends on most recent
