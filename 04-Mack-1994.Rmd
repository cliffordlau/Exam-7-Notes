...
* CI is too small for older ages as there are only a few LDFs (and too large for younger ages)
...
Use rank correlation (e.g. Spearman's correlation) instead of value correlation (e.g. Person correlation)
...
1. Calculate Spearman's correlation for each pair of **adjacent LDFs**

$$S_k = \sum \limits_{i} \Big \{ rank(f_{i,k-1}) - rank(f_{i,k}) \Big \}^2$$

$$T_k = 1 - \dfrac{S}{n_k(n_k^2-1)/6}$$
...
2. Calculate $T$ for the whole triangle
