...
1. [Diagnose](#era1-diagnose): Assess all risk and considergeneral environment, industry, and firm specific items

2. [Analyze](#era1-analyze) what risk are critical and consider correlations; Select measures consistent with management's veiw of risk and prioritize
...
* Cat Model
    ...
    * Nowadays it would be considered mismangement for a company not to use cat models
...
5. **Retention** (by desing of default) some or all of the risk
...
* Determining capital need (to support riks, maintain rating, etc)
...
* Can be modeled in bulk using informed judgements (but with high degree of uncertainty)
...
* **Estimation Risk**:  
Error in estimatins the form and parameters
...
Error on forecast of future value even if we fit the historical data accurately
    ...
    * Examples:
        ...
        * Developemen of ultimate losses
    
    * Affected by macro factors (e.g. inflation) and the coresponding dependencies should be reflected in the model
...
Causal link between a large unpredictable event and losses to an insurer
    ...
    * Exposure that existed, unknown, for many years comes to light (e.g. asebestos)
...
* Based on proprietary cat models, further source of uncertaity

    * Differences between commercial models and different versions of the same model
    
    * Considerable amount of uncertainty in the probabilities of various events and the amount of insured damange
